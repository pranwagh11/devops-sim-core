...
const SCHEMA_VERSION_KEY = "devops-sim:schema-version";

// Bump this whenever a record shape changes in a way that makes old
// localStorage data incompatible with the current app (e.g. adding a
// required field like lesson_id). This prototype does not migrate old
// data forward — it wipes it and lets the bundled seed repopulate
// cleanly, which is a deliberate, documented tradeoff at this stage
// rather than an oversight. See README "Known limitations".
...
function hasAnyContent(): boolean {
  return (
    readAll<ModuleRecord>(MODULES_KEY).length > 0 ||
    readAll<LessonRecord>(LESSONS_KEY).length > 0 ||
    readAll<ChallengeRecord>(CHALLENGES_KEY).length > 0 ||
    readAll<ProjectRecord>(PROJECTS_KEY).length > 0
  );
}

// Wipes content if it was written by an older, incompatible schema. This
// is what fixes the exact symptom of "seed file loads over the network
// but nothing shows up" — that happens when leftover pre-Module-hierarchy
// challenges (with no lesson_id) already occupy the challenges key, which
// blocks the normal "seed only if empty" logic from ever running.
function migrateSchemaIfNeeded(): void {
  ...
  if (hasAnyContent()) {
    console.warn(
      `[devops-sim] Local data was written by an older, incompatible version of this app ` +
      `(schema v${storedVersion} vs current v${CURRENT_SCHEMA_VERSION}) and has been cleared so ` +
      `the bundled seed data can load cleanly. This prototype does not migrate old authored ` +
      `content forward across schema changes — see README.`
    );
  }
...
export const storage = {
  ...
  // Manual, admin-triggered reset (distinct from the automatic
  // schema-version wipe above) — wipes all authored content but leaves the
  // schema version stamp and the admin credential file untouched.
  resetLocalData(): void {
    localStorage.removeItem(MODULES_KEY);
    localStorage.removeItem(LESSONS_KEY);
    localStorage.removeItem(CHALLENGES_KEY);
